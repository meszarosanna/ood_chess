...
class ActionValueEngine(NeuralEngine):
  ...
  def analyse(self, board: chess.Board) -> engine.AnalysisResult:
    ...
    #legal_dummy_return_buckets = np.zeros((len(legal_actions), 1), dtype=np.int32)
    # Tokenize the board.
    tokenized_fen = tokenizer.tokenize(board.fen()).astype(np.int32)
    #legal_sequences = np.stack([tokenized_fen] * len(legal_actions))
    # Create the sequences.
    #legal_sequences = np.concatenate(
    #    [legal_sequences, legal_actions, legal_dummy_return_buckets],
    #    axis=1,
    #)
    ...
    total_action_log_probs = self.predict_fn(sequences)[:, -1]

    legal_actions = np.squeeze(legal_actions)
  ...
  def play(self, board: chess.Board) -> chess.Move:
    return_buckets_log_probs = self.analyse(board)['legal_log_probs']
...
class BCEngine(NeuralEngine):
  ...
  def play(self, board: chess.Board) -> chess.Move:
    action_log_probs = self.analyse(board)['total_log_probs']
