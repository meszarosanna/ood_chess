...
def get_best_move(fen_string, neural_engine, win_probab=False, legal=True):
    ...
    start = time.time()  
    best_move = neural_engine.play(board, legal)
    end = time.time()
    print(f'Best move: {best_move}')
    print(f'Time taken: {end - start} seconds')
    #print(f'Legal best move: {legal_best_move}')
    return best_move
...
if __name__ == "__main__":
    ...
    fen = "k7/R7/R7/8/8/8/8/8 b"
    #INPUT_FILE = 'searchless_chess/data/puzzles.csv' #'ood_puzzles.csv'
    #_NUM_PUZZLES = 10
    ...
    get_best_move(fen, model, win_probab, legal)
